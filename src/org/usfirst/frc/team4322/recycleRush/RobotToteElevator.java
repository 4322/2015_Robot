		...
		try
		{
			...
			if(!setPointSelectButtonPressed)
			{
				...
				else if(CoPilotController.getInstance().getElevatorSetPointContainerButton())
				{
					...
					setPointDelta = setPointChange.CONTAINER;//going to an arbitrary index; not incrementing or decrementing
				}
			}
	...
	public void autoDrive()
	{
		...
		if(newSetpoint)
		{
			...
			else if(setPointDelta == setPointChange.CONTAINER)
			{
				...
				if(containerModeTargetIndex > RobotMap.ELEVATOR_CONTAINER_POSITIONS.length - 1)
					containerModeTargetIndex = 0;
			}
			//if stacking
